...
for datafile in os.listdir(directory):	
    ...
    if hdffile.endswith(".he5"):
        ...
        #print(dateindex)
        # add data to list
        data = np.array([f[name][:,subset][box_index]*10**9, latitude[box_index], longitude[box_index], dateindex*len(latitude[box_index]), quality[box_index], convergence[box_index], year*len(latitude[box_index])], dtype=np.float32)
        #print(data)
...
for i in range(len(data_arr[0])):
    for j in range(7):
        if data_arr[0][i]>0 and data_arr[5][i]<=2 and data_arr[4][i]>=1:
...
os.chdir('path_to_data_dir'+loc_pressure)
