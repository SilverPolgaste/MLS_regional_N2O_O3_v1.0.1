...
for file in os.listdir(directory):	
    ...
    if hdffile.endswith(".he5"):
        ...
        name = '/HDFEOS/SWATHS/N2O/Data Fields/L2gpValue'
        month = '/HDFEOS/ADDITIONAL/FILE_ATTRIBUTES[GranuleMonth]'
        # get the geolocation data
        latitude = f['/HDFEOS/SWATHS/N2O/Geolocation Fields/Latitude'][:]
        longitude = f['/HDFEOS/SWATHS/N2O/Geolocation Fields/Longitude'][:]
        # get quality flags
        quality = f['/HDFEOS/SWATHS/N2O/Data Fields/Quality'][:] 
        convergence = f['/HDFEOS/SWATHS/N2O/Data Fields/Convergence'][:] 
        # define box
        ...
        fullindex=[year[0]+dateindex[0]]
        #print(dateindex)
        # add data to list
        data = np.array([f[name][:,subset][box_index]*10**9, latitude[box_index], longitude[box_index], fullindex*(len(latitude[box_index])), quality[box_index], convergence[box_index]], dtype=np.float32)
        #print(data)
...
for i in range(len(data_arr[0])):
    for j in range(6):
        if data_arr[0][i]>0 and data_arr[5][i]<2 and data_arr[4][i]>0.8:
